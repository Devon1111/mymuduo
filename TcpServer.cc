...
TcpServer::TcpServer(EventLoop* loop,
    ...
    , connectionCallback_(/*code*/)
    , messageCallback_(/*code*/)
    , nextConnId_(1)
    , started_(0)
{
    //当有新用户连接时，会执行TcpServer::newConnection回调
    acceptor_->setNewConnectCallbace(std::bind(&TcpServer::newConnection, this
...
TcpServer::~TcpServer()
{
    ...
    {
        //这个局部的shared_ptr，在出右括号后，可以自动释放new出来的TcpConnection对象资源
...
void TcpServer::newConnection(int sockfd, const InetAddress& peeraddr)
{
    ...
    //根据连接成功的sockfd，创建TcpConnection连接
...
{
    LOG_INFO("Tcp::removeConnectionInLoop [%s] - connection %s\n", name_.c_str(), conn->name().c_str());
